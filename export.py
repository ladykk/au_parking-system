...
def export_openvino(model, file, half, prefix=colorstr('OpenVINO:')):
    ...
    try:
        ...
        with open(Path(f) / file.with_suffix('.yaml').name, 'w') as g:
            # add metadata.yaml
            yaml.dump({'stride': int(max(model.stride)),
                      'names': model.names}, g)
...
def export_saved_model(model,
    ...
    try:
        ...
        tf_model = TFModel(cfg=model.yaml, model=model,
                           nc=model.nc, imgsz=imgsz)
        ...
        if keras:
            ...
            tfm.__call__ = tf.function(lambda x: frozen_func(
                x)[:4] if tf_nms else frozen_func(x)[0], [spec])
            tfm.__call__(im)
...
def export_edgetpu(file, prefix=colorstr('Edge TPU:')):
    ...
    try:
        ...
        if subprocess.run(f'{cmd} >/dev/null', shell=True).returncode != 0:
            ...
            sudo = subprocess.run(
                'sudo --version >/dev/null', shell=True).returncode == 0
            for c in (
...
def run(
    ...
    if engine:  # TensorRT required before ONNX
        f[1] = export_engine(model, im, file, train, half,
